...
import { updateSurveyJobAction } from '@/app/(dashboard)/jobs/survey/actions'
import type { SurveyJob, Client } from '@/types/database'

export default async function EditSurveyJobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: job }, { data: clients }] = await Promise.all([
    db
      ...
      .select('*')
      .eq('id', id)
      .single() as unknown as Promise<{ data: SurveyJob | null }>,
    db
      .from('clients')
  ...
  const action = updateSurveyJobAction.bind(null, id)
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Survey Job</h1>
